...
class JsonifyTest(unittest.TestCase):
    ...
    def test_camel_casing(self):
        ...
        self.assertEqual("hypens-confuse-me",
                         camel_casify("hypens-confuse-me"))
        self.assertEqual("trailingDoesntMatter_",
                         camel_casify("trailing_doesnt_matter_"))
    ...
    def test_camel_casing_encoder(self):
        self.assertEqual(
            ('{"oneTwo": ["buckle", "shoe"],'
             ' "threeFour": {"knock": "door"},'
             ' "fivesix": "pickup sticks"}'),
            JSONModelEncoderCamelCased().encode(self.o))

    def test_xss_prevention(self):
        self.assertEqual(
            ('{"bar": ["foo<\\/scr<\\/script>ipt>"],'
             ' "foo": "foo<\\/script>",'
             ' "baz": "foo<\\/script   >"}'),
             JSONModelEncoder().encode({
                "foo": "foo</script>",
                "bar": ["foo</scr</script>ipt>"],
                "baz": "foo</script   >"
             })
