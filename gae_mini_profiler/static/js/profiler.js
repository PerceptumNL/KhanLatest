...
var GaeMiniProfiler = {
    ...
    modes: ["cpu_only", "rpc_only", "cpu_and_rpc", "simple"],
    ...
    /**
     * Set the profiler to the next mode in our list of possible modes.
     * This will not change anything for the current request. The page must be
     * reloaded after the mode is changed.
     */
    nextMode: function(link) {

        // Find mode coming up after the current cookie's mode value
        var nextIndex = $.inArray(this.getCookieMode(), this.modes) + 1;
        if (nextIndex >= this.modes.length) {
            nextIndex = 0;
        }

        var mode = this.modes[nextIndex];

        // Set mode cookie for profiler to detect on next request
        $.cookiePlugin("g-m-p-mode", mode, {path: '/'});
        $(link).text(this.modeDescription(mode));
    },
    ...
    /**
     * Return a user-friendly description of each profiler mode.
     */
    modeDescription: function(mode) {
        switch (mode) {
            case "cpu_only":
                return "On (CPU only)";
            case "rpc_only":
    ...
    expand: function(elEntry, data) {
        ...
        jPopup
            ...
            .find(".next-mode")
                .text(this.modeDescription(this.getCookieMode()))
                .click(function() { GaeMiniProfiler.nextMode(this); return false; }).end()
