...
def class_time_graph_context(user_data, dt_utc, tz_offset, student_list):
    ...
    if classtime_analyzer.timezone_offset != -1:
        # If no timezone offset is specified, don't bother grabbing
        # all the data because we'll be redirecting back to here w/
        # timezone information.
        import os
        try:
            report_method = setting_model.Setting.classtime_report_method()
            report_startdate = datetime.datetime.strptime(
                setting_model.Setting.classtime_report_startdate(),
                "%Y-%m-%dT%H:%M:%S")

            if (os.environ["QUERY_STRING"].find("&version=2") != -1 or
                    int(report_method) == 2 and
                    report_startdate < dt_utc):
                classtime_table = \
                    classtime_analyzer.get_classtime_table_by_coach(
                        user_data, students_data, dt_utc)
            else:
                classtime_table = classtime_analyzer.get_classtime_table_old(
                    students_data, dt_utc)
        except Exception, e:
    ...
    for user_data_student in students_data:
        ...
        if classtime_table is not None:
            total_student_minutes = classtime_table.get_student_total(
                user_data_student.email)
