...
# TODO(chris): convert to an end-to-end test that runs as part of the
# test suite
...
def test_response(step, data={}):
    ...
    if last_opener is None:
        ...
        last_opener.open("http://localhost:8080/_ah/login"
                         "?email=test%40example.com&admin=True&action=Login"
                         "&continue=%2Fpostlogin")

    data["step"] = step

    req = last_opener.open("%s?%s" % (TEST_GANDALF_URL,
...
def run_tests():
    ...
    assert (test_response("can_cross_all_users_whitelist_and_blacklist") ==
            False)
