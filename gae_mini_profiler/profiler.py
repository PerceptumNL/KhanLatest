...
def id_for_current_request():
    # a per-request identifier accessed by a couple other pieces of
    # gae_mini_profiler. It is managed as part of the middleware lifecycle.
    return _local.request_id

class Mode(object):
    """Enum types of possible profiler modes."""

    SIMPLE = "simple"  # Simple start/end timing for the request as a whole
    CPU_ONLY = "cpu_only"  # Profile all function calls
    RPC_ONLY = "rpc_only"  # Profile all RPC calls
    CPU_AND_RPC = "cpu_and_rpc"  # Profile all function and RPC calls
...
class RequestStats(object):
    ...
    def __init__(self, request_id, environ, profiler):
        ...
        self.profiler_results = RequestStats.calc_profiler_results(profiler)
        self.appstats_results = RequestStats.calc_appstats_results(profiler)
    ...
    @staticmethod
    def seconds_fmt(f, n=5):
        return RequestStats.milliseconds_fmt(f * 1000, n)

    @staticmethod
    def milliseconds_fmt(f, n=5):
        return ("%f" % round(f, n)).rstrip("0").rstrip(".")
    ...
    @staticmethod
    def calc_profiler_results(profiler):

        if profiler.mode == Mode.SIMPLE or profiler.mode == Mode.RPC_ONLY:
            return {
                "total_time": RequestStats.seconds_fmt(profiler.end - profiler.start, 0),
            }
        ...
        stats = pstats.Stats(profiler.prof, stream=output)
    ...
    @staticmethod
    def calc_appstats_results(profiler):

        if not profiler.recorder:
            return {
                "total_time": 0
            }
        ...
        appstats_key = long(profiler.recorder.start_timestamp * 1000)

        for trace in profiler.recorder.traces:
            total_call_count += 1
...
class ProfilerWSGIMiddleware(object):
    ...
    def __call__(self, environ, start_response):
        ...
        if config.should_profile(environ) and not environ.get("PATH_INFO", "").startswith("/gae_mini_profiler/"):
            ...
            import base64
            _local.request_id = base64.urlsafe_b64encode(os.urandom(5))
            ...
            def profiled_start_response(status, headers, exc_info = None):
                ...
                headers.append(("X-MiniProfiler-Id", _local.request_id))
                headers.append(("X-MiniProfiler-QS", environ.get("QUERY_STRING")))

                return start_response(status, headers, exc_info)

            profiler = RequestProfiler(_local.request_id, Mode.get_cookie_mode())
    ...
    def headers_with_modified_redirect(environ, headers):
        ...
        for header in headers:
            if header[0] == "Location":
                ...
                request_id_chain = id_for_current_request()
