...
from third_party.flask import current_app, request, redirect, session
from third_party.flask.session import Session
...
def access_token_response(oauth_map):
    ...
    return "oauth_token=%s&oauth_token_secret=%s" % (
        oauth_map.access_token, oauth_map.access_token_secret)
...
def custom_scheme_redirect(url_redirect):
    ...
    scheme_lists = [urlparse.uses_netloc, urlparse.uses_query,
                    urlparse.uses_fragment, urlparse.uses_params,
...
def current_oauth_map_from_session_unsafe():
    ...
    # We have to use plain 'ole cookie handling before we switch over
    # to a Flask-only app, at which point we can strictly rely on
    # Flask sessions.
    ...
    if session_cookie_value and App.flask_secret_key:
        ...
        session_cookie = Session.load_cookie(fake_request, session_cookie_name,
                                             secret_key=App.flask_secret_key)
        if session_cookie and "oam" in session_cookie:
...
def get_response(url, params={}):
    ...
    while (not result or result.status_code == 503) and c_tries_left > 0:
        ...
        try:
            ...
            c_tries_left -= 1
            logging.warning("Trying to get response for %s again (tries "
                            "left: %s) due to error: %s" %
                            (url, c_tries_left, e.message))
    ...
    if result:
        ...
        if result.status_code == 200:
            ...
            # Probably user denied access to our app; we'll catch this
            # and show a nice error
            raise OAuthBadRequestError("Bad request for url %s" % url)
        else:
            logging.warning(result.content)
            raise OAuthError("Error in get_response, received status %s "
                             "for url %s" % (result.status_code, url))
