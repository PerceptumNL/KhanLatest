...
# LastActionCache stores a highly compressed cache of the most recent
# actions each individual user has taken. The data is stored in
# memcache only at the moment for performance reasons. If reliability
# becomes an issue we can consider persisting this data in the
# datastore in a background thread.
#
# LastActionCache is used to quickly assess badge completion based on
# each users' recent actions.
#
# LastActionCache explicitly stores only the protobuf versions of each
# cached action for extremely fast serialization and deserialization
# of the action cache to and from memcache on every action request.
#
# We only take the time to turn each protobuf instance into a real DB
# model on demand (when needed for badge assessment).
...
class LastActionCache:
    ...
    def __init__(self, user_data):
        # Protobuf version of problem logs for extremely fast (de)serialization
        self.problem_logs = []

        # Deserialized problem log models
        self.problem_log_models = {}

        # Protobuf version of video logs for extremely fast (de)serialization
        self.video_logs = []

        # Deserialized video log models
    ...
    def get_problem_log(self, ix):
        if ix not in self.problem_log_models:
            self.problem_log_models[ix] = db.model_from_protobuf(
                entity_pb.EntityProto(self.problem_logs[ix]))
    ...
    def get_video_log(self, ix):
        if ix not in self.video_log_models:
            self.video_log_models[ix] = db.model_from_protobuf(
                entity_pb.EntityProto(self.video_logs[ix]))
