...
class BadgeStat(db.Model):
    ...
    def needs_update(self):
        """Update this badge's stats if it's been at least 6 hours
        since last calculation.

        This code runs in a task queue, so if the task fails and
        requeues itself this protects us from multiple updates per
        badge in a short time frame.
        """
    ...
    def update(self):
        ...
        if self.dt_last_calculated:
            ...
            dt_last_calculated_next = min(
                    dt_last_calculated_next,
                    self.dt_last_calculated + datetime.timedelta(days=10))
...
class CustomBadgeType(db.Model):
    ...
    def insert(name, description, full_description, points, badge_category,
        ...
        if (not name or not description or not full_description or
                points < 0 or badge_category < 0):
            return None
...
class UserBadge(backup_model.BackupModel):
    ...
    def get_for(user_data):
        ...
        query.order('-date')
        return query.run(batch_size=1000)
