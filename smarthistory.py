...
def deleteOldBlobs():
    ...
    for blob in blobs:
        if blob.filename.find(SMARTHISTORY_URL) != -1:
            ...
            if (age.days * 86400 + age.seconds >=
                    SMARTHISTORY_IMAGE_CACHE_EXPIRATION):
                blob.delete()


class SmartHistoryProxy(request_handler.RequestHandler,
    ...
    def get(self):
        ...
        if "clearcache" in self.request.params:
            self.clearCache()
        ...
        #redirect file back to smarthistory.org if the file is an
        #audio/video and hence might be too big to store in blobstore
        extension = self.request.path[self.request.path.rfind(".") + 1:]
        if extension.lower() in ("mp3", "m4a", "flv", "mp4", "mov", "avi",
                                 "m4v", "swf"):
            logging.info("multimedia: sending redirect request back to "
                         "Smarthistory for %s" % self.request.path)
        ...
        if "Location" in response_headers:
            logging.info("sending redirect request back to Smarthistory for %s"
                         % self.request.path)
            self.redirect(response_headers["Location"], True)
            return
        
        #might need to convert all dictionary keys to lower case
        #check to see if we need to return 304 - might need to add handling
        #of If-Modified-Since, in case If-None-Match is not sent 
        if ("If-None-Match" in self.request.headers and
                "etag" in response_headers and
                (self.request.headers["If-None-Match"] ==
                 response_headers["etag"])):
            logging.info("sending 304 from within the application")

            slash_index = response_headers.get("content-type").find("/")
            content_type = response_headers.get("content-type")[:slash_index]
            ...
            if content_type in ("image", "audio", "video"):
                ...
                self.response.headers["Cache-Control"] = (
                    "public, max-age=%s;" % SMARTHISTORY_CACHE_EXPIRATION_TIME)
    ...
    def post(self):
        ...
        try:
            response = urlfetch.fetch(
                url=SMARTHISTORY_URL + self.request.path,
    ...
    def load_resource(self):
        ...
        logging.info("getting resource %s from %s" % (path, SMARTHISTORY_URL))
        try:
            response = urlfetch.fetch(url=SMARTHISTORY_URL + path,
                                      headers=headers, deadline=25)
            ...
            raise SmartHistoryLoadException(
                "Failed loading %s from SmartHistory with Exception: %s" %
                (path, e))
        ...
        if response.status_code != 200:
            ...
            if response.status_code == 404:
                raise SmartHistoryLoadException(
                    "After attempt #%i Failed loading %s from SmartHistory "
                    "with response code:%i " % (
                        self.attempt_counter, path, response.status_code))  
        ...
        #load the response headers into a dictionary as layer_cache
        #was throwing an error caching an object of class
        #mimetools.Message
        response_headers = dict((h, response.headers[h])
                                for h in response.headers
                                if h not in ["Content-Length", "Host"]) 

        slash_index = response.headers.get("content-type").find("/")
        content_type = response.headers.get("content-type")[:slash_index]

        #check to see if it is an image, audio, or video, if so store
        #it in blobstore, set the appropriate headers and remove data
        #from the output
        if content_type in ("image", "audio", "video"):
            ...
            file_name = files.blobstore.create(
                ...
                _blobinfo_uploaded_filename=SMARTHISTORY_URL + path)
  
            # Writing too large a chunk to the blobstore at a single
            # time throws an error, so it should be done in pieces
            pos = 0
            chunkSize = 65536
            ...
            response_headers["Cache-Control"] = (
                "public, max-age=%s;" % SMARTHISTORY_IMAGE_CACHE_EXPIRATION)
