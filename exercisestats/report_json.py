...
class ExerciseOverTimeGraph(request_handler.RequestHandler):
    ...
    # TODO(david): What's the best way to deal with the wrapped function having
    # default values?
    ...
    def exercise_over_time_for_highcharts(self, exids, dates, title='', showLegend=False):
        ...
        def normalize_for_highcharts(data_list):
            # It's possible that the ExerciseStats that we go through has
            # multiple entries for a particular day (eg. if we were aggregating
            # more than one exercise). Sum them.
            data_list = sum_keys(data_list)

            # June 3, 2012: Highcharts now draws line graphs following the order
            # that data values are given, so sort the lists by time (first item)
            return sorted(data_list)

        done_list = normalize_for_highcharts(done_list)
        prof_list = normalize_for_highcharts(prof_list)
        new_users_list = normalize_for_highcharts(new_users_list)
        ...
        # TODO(david): Call a function to render all values in this dict as JSON
...
# TODO(david): Either allow returning graphs for other statistics, such as #
...
class ExerciseStatsMapGraph(request_handler.RequestHandler):
    # TODO(david): Just move this logic into get and make get_use_cache take
    #     a day parameter.
...
class ExerciseNumberTrivia(request_handler.RequestHandler):
    ...
    def number_facts_for_geckboard_text(number):
        ...
        def get_fact_from_api(fact_type, query_string=''):
            api_url = 'http://numbersapi.com/%s/%s%s' % (number, fact_type,
                    query_string)
            return urllib2.urlopen(api_url).read()

        trivia_fact = get_fact_from_api('trivia', '?notfound=floor&fragment')
        trivia_fact = 'We now have more exercises than %s!' % trivia_fact

        math_fact = get_fact_from_api('math')
        year_fact = get_fact_from_api('year')

        return {
            'item': [
                { 'text': cgi.escape(trivia_fact) },
                { 'text': cgi.escape(math_fact) },
                { 'text': cgi.escape(year_fact) },
            ]
        }
