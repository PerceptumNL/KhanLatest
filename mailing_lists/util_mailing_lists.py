...
def constant_contact_request(url, xml_payload=None, put_request=False):
    ...
    try:
        if xml_payload:
            request_entity = urllib2.Request(url, xml_payload,
...
def create_new_constant_contact(email, list_url):
    ...
    contacts_url = ("https://api.constantcontact.com/ws/customers/%s/contacts" %
    ...
    # Constant Contact passes back "201 Created" response for
    # successful creation.
...
def get_constant_contact_url(email):
    ...
    search_url = ("https://api.constantcontact.com/ws/customers/%s/contacts?email=%s" %
                  (CONSTANT_CONTACT_USERNAME, urllib.quote(email)))
...
def add_list_to_constant_contact(email, list_url):
    ...
    node_new_list = (minidom
                     .parseString("<ContactList id=\"%s\" />" % list_url)
                     .firstChild)
    node_contact_lists.appendChild(node_new_list)

    constant_contact_request(contact_url, node_contact.toxml(), True)
...
def subscribe_to_constant_contact(email, list_id):
    ...
    list_url = ("https://api.constantcontact.com/ws/customers/%s/lists/%s" %
                (CONSTANT_CONTACT_USERNAME, list_id))
