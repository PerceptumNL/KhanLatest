...
from oauth_provider.oauth import (OAuthConsumer, OAuthToken, OAuthRequest,
                                  OAuthSignatureMethod_HMAC_SHA1)


class GoogleOAuthClient(object):

    Consumer = OAuthConsumer(App.google_consumer_key,
                             App.google_consumer_secret)

    def fetch_request_token(self, oauth_map):
        oauth_request = OAuthRequest.from_consumer_and_token(
                GoogleOAuthClient.Consumer,
                http_url="http://www.iktel.nl/_ah/OAuthGetRequestToken",
        ...
        oauth_request.sign_request(OAuthSignatureMethod_HMAC_SHA1(),
                                   GoogleOAuthClient.Consumer, None)
    ...
    def fetch_access_token(self, oauth_map):
        ...
        token = OAuthToken(oauth_map.google_request_token,
                           oauth_map.google_request_token_secret)

        oauth_request = OAuthRequest.from_consumer_and_token(
                GoogleOAuthClient.Consumer,
                token=token,
                verifier=oauth_map.google_verification_code,
                http_url="http://www.iktel.nl/_ah/OAuthGetAccessToken"
                )
