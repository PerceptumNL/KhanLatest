...
def get_inline_template(package_name, file_name):
    ...
    return ("<script type='text/x-handlebars-template' "
            "id='template_%s-package_%s'>%s</script>") % (package_name,
                                                          name, contents)
...
def js_dynamic_package(package_name):
    list_js = ["function dynamicLoadPackage_%s(callback) {" % package_name,
               "dynamicPackageLoader.load('%s', callback, [" % package_name]
    if not use_compressed_packages():
        package = packages.javascript[package_name]
        base_url = (package.get("base_url") or
                    "/javascript/%s-package" % package_name)
