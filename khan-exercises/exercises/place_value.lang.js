({
	"nl" : {
		...
		"question2" : "Hoe schrijf je  <var>cardinal( NUMBER )</var> in cijfers?",
		...
		"hint4" : '(function() {'+
					...
					'				if ( DIGITS[ index - 1] === 0 ) {'+
					'					ADDENDS.push( digit );'+
					'					return cardinal( digit ) + "len is hetzelfde als " + digit;'+
					'				}'+
					'			} else if ( index === DIGITS.length - 2 ) {'+
					'				var addend = digit * 10 + DIGITS[ DIGITS.length - 1 ];'+
					'				ADDENDS.push( addend );'+
					'				return cardinal( addend ) + " is hetzelfde als " + addend;'+
					'			} else {'+
					'				var words = cardinal( digit ) + " keer een " + powerToPlace(maxPower - index);'+
